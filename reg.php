...
if (isset($_POST['submit'])) {
  ...
  if ($password != $repassword) {
    ...
    // Check if the mess name or email already exists
    ...
    if ($result && $result->num_rows > 0) {
      ...
      if ($row['messname'] == $uname) {
        $msg = "<div class='alert alert-danger'><span>This Mess Name Is Already Exists. Please Try A Different Name For Registration!</span></div>";
      } elseif ($row['email'] == $email) {
        $msg = "<div class='alert alert-danger'><span>This Email Is Already Exists. Please Try A Different Email For Registration!</span></div>";
      }
    } else {
      // Register the user if credentials are fine
      $query1 = "INSERT INTO reg (email, password, messname) VALUES ('$email', '$password', '$uname')";
      $result = $ob->insert($query1);

      if ($result) {
        $msg = "<div class='alert alert-success'><span>Registration Successful! You can now <a href='m_index.php'>Login</a>.</span></div>";
      } else {
        $msg = "<div class='alert alert-danger'><span>Registration Failed. Please Try Again!</span></div>";
      }
    }
  }
}
    ...
    <div class="form2 slide">
      ...
      <form action="reg.php" method="post">
        <div class="form-group">
          ...
          <div class="form-group">
            <a href="m_index.php">Already Registration?Login</a>
          </div>
      </form>
    </div>
