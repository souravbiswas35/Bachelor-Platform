...
<div id="print">
								<table border="1"  class="display table table-striped table-bordered table-hover" cellspacing="0" width="100%"  >
									...
									<?php 
$bid=intval($_GET['bid']);
									$sql = "SELECT users.*,area.BrandName,seats.SeatTitle,booking.FromDate,booking.ToDate,booking.message,booking.VehicleId as vid,booking.Status,booking.PostingDate,booking.id,booking.BookingNumber,
DATEDIFF(booking.ToDate,booking.FromDate) as totalnodays,seats.PricePerMonth
									  from booking join seats on seats.id=booking.VehicleId join users on users.EmailId=booking.userEmail join area on seats.SeatArea=area.id where booking.id=:bid";
$query = $dbh -> prepare($sql);
	...
	<h3 style="text-align:center; color:red">#<?php echo htmlentities($result->BookingNumber);?> Booking Details </h3>
		...
		<tr>
										...
										<tr>
											...
											<tr>											
											<th>Vehicle Name</th>
											<td><a href="edit-seat.php?id=<?php echo htmlentities($result->vid);?>"><?php echo htmlentities($result->BrandName);?> , <?php echo htmlentities($result->SeatTitle);?></td>
											<th>Booking Date</th>
